...
app.get('/api/realtime/stream', (req, res) => {
  ...
  sseClients.push(res);
  console.log(`[Realtime:SSE] Client registered. Current active connections: ${sseClients.length}`);

  req.on('close', () => {
    sseClients = sseClients.filter(client => client !== res);
    console.log(`[Realtime:SSE] Client disconnected. Current active connections: ${sseClients.length}`);
  });
});

app.post('/api/realtime/broadcast', (req, res) => {
  const { event, payload } = req.body;
  console.log(`[Realtime:Broadcast] Event: ${event}, Broadcasters count: ${sseClients.length}`);
...
function getRealRedirectUrl(req: any, clientUrl: string, defaultPath: string): string {
  ...
  console.log(`[Stripe URL Redirect Resolution] Deduced origin: "${origin}" for clientUrl: "${clientUrl}"`);

  if (clientUrl) {
    if (clientUrl.startsWith('http://') || clientUrl.startsWith('https://')) {
      if (!clientUrl.includes('localhost:') && !clientUrl.includes('127.0.0.1')) {
        console.log(`[Stripe URL Redirect Resolution] Returning absolute public redirect URL: "${clientUrl}"`);
        return clientUrl;
      }
    }
    try {
...
app.post('/api/stripe/connect/onboard', async (req, res) => {
  try {
    ...
    if (!stripeAccountId) {
      ...
      if (updateErr) {
        console.error('Failed to save stripe_account_id to database:', updateErr);
      }
    }

    // Generate onboarding links
    const returnUrl = getRealRedirectUrl(req, `/dashboard?status=connect_success&stripe_acct=${stripeAccountId}&biz_id=${businessId}`, `/dashboard`);
    const refreshUrl = getRealRedirectUrl(req, `/dashboard?status=connect_refresh&biz_id=${businessId}`, `/dashboard`);
...
app.post('/api/create-checkout-session', async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('[Checkout ID Lookup Backup] Non-blocking exception:', dbLookupErr.message);
    }
...
async function getOrCreatePriceIdFallback(stripe: Stripe): Promise<string> {
  console.log("[Stripe Fallback] Attempting to find or create 'Glamzo PRO Plan' product and price on this account...");
  try {
    const productsList = await stripe.products.list({ limit: 100 });
    let product = productsList.data.find(p => p.name === 'Glamzo PRO Plan' && p.active);
    if (!product) {
      console.log("[Stripe Fallback] 'Glamzo PRO Plan' product not found. Creating a new product...");
      product = await stripe.products.create({
        name: 'Glamzo PRO Plan',
        description: 'Gestor operacional, faturamento de reservas, campanhas de marketing e fidelização de clientes - Glamzo PRO SaaS.'
      });
    }

    const pricesList = await stripe.prices.list({ product: product.id, limit: 100, active: true });
    // Look for recurring monthly price with amount 1990 cents (19.90 EUR)
    let price = pricesList.data.find(p => p.unit_amount === 1990 && p.recurring?.interval === 'month' && p.currency === 'eur');
    if (!price) {
      console.log("[Stripe Fallback] Recurring price not found for product. Creating monthly 19.90 EUR price...");
    ...
    console.log(`[Stripe Fallback] Successfully resolved fallback Price ID: ${price.id}`);
    return price.id;
  } catch (err: any) {
    console.error("[Stripe Fallback Error] Tried to resolve fallback price but custom product/price bootstrap failed:", err.message);
    throw err;
  }
}
...
const handleCreateSubscriptionCheckout = async (req: any, res: any) => {
  ...
  try {
    ...
    if (!businessId) {
      ...
      res.status(400).json({ success: false, error: 'Missing businessId parameter' });
      return;
    }
    ...
    if (!secretKey) {
      console.error("[Stripe Checkout Error] Stripe secret key is missing inside server configuration.");
      res.status(500).json({ success: false, error: 'Stripe key missing: STRIPE_SECRET_KEY is not defined in server environment.' });
      return;
    }
    ...
    try {
      ...
      console.error("[Stripe Checkout Error] Stripe initialization failed:", stripeInitErr);
      res.status(500).json({ success: false, error: 'Stripe API failed: Initialization error - ' + stripeInitErr.message });
      return;
    }
    ...
    if (publishableKey) {
      ...
      if ((isSkLive && !isPkLive) || (isSkTest && !isPkTest)) {
        console.warn('⚠️ Stripe environment warning: Mixing TEST and LIVE keys!');
      }
    }
    ...
    if (bizErr) {
      console.error("[Stripe Checkout Error] Database failed reading business profile:", bizErr.message);
      res.status(500).json({ success: false, error: 'Database failed reading business: ' + bizErr.message });
      return;
    }

    if (!business) {
      console.error("[Stripe Checkout Error] Business profile not found for id:", businessId);
      res.status(404).json({ success: false, error: 'Invalid partner: Business profile not found.' });
      return;
    }
    ...
    if (!customerId) {
      try {
        ...
        console.error("[Stripe Checkout Error] Stripe customer creation failed:", custErr.message);
        res.status(500).json({ success: false, error: 'Invalid customer: Stripe customer create failed - ' + custErr.message });
        return;
      }
    }
    ...
    try {
      ...
      const isNoSuchPrice = errMsg.toLowerCase().includes("no such price") || 
      ...
      if (isNoSuchPrice) {
        console.warn(`[Stripe Automatic Recovery] Configured priceId '${priceId}' failed or was empty. Error detail: "${errMsg}". Running automatic product & price bootstrap fallback...`);
        try {
          const fallbackPriceId = await getOrCreatePriceIdFallback(stripe);
          console.log(`[Stripe Automatic Recovery] Retrying session creation with resolved fallback priceId: '${fallbackPriceId}'`);
          session = await stripe.checkout.sessions.create({
            customer: customerId,
            mode: 'subscription',
            line_items: [
          ...
          console.error("[Stripe Automatic Recovery Failure] Auto-recovery bootstrap also failed:", fallbackErr.message);
          res.status(500).json({ 
            success: false, 
        ...
        console.error("[Stripe Checkout Error] Stripe Session creation failed:", checkoutErr.message);
        res.status(500).json({ success: false, error: 'Stripe API failed: Session creation error - ' + checkoutErr.message });
        return;
      }
    }
    ...
    console.error('Stripe SaaS Subscription Checkout generation completely failed:', err);
    res.status(500).json({ success: false, error: err.message || 'Unknown internal backend exception' });
  }
};
...
app.post('/api/stripe/verify-subscription', async (req, res) => {
  try {
    ...
    if (sessionId) {
      console.log(`[Verify Subscription] Fetching Stripe checkout session: ${sessionId}`);
    ...
    if (!resolvedBusinessId) {
      res.status(400).json({ error: 'Could not resolve businessId from request or session metadata.' });
      return;
    }

    // Fallback: If no sessionId was provided or no subscription link was found, try finding via stripe_customer_id
    if (!subId || !customerId) {
      ...
      if (biz && biz.stripe_customer_id) {
        customerId = biz.stripe_customer_id;
        console.log(`[Verify Subscription Fallback] Listing active subscriptions for Stripe customer: ${customerId}`);
    ...
    if (!subId) {
      res.status(404).json({ error: 'Nenhuma subscrição ativa encontrada no Stripe para este salão.' });
      return;
    }

    // Retrieve active subscription details directly from Stripe
    console.log(`[Verify Subscription] Retrieving finalized Stripe subscription details for ID: ${subId}`);
    const stripeSub = await stripe.subscriptions.retrieve(subId);
    const subStatus = (stripeSub as any).status;
    const expiresAt = new Date((stripeSub as any).current_period_end * 1000).toISOString();

    const currentStatus = subStatus === 'trialing' ? 'trialing' : subStatus;
    const isSActive = currentStatus === 'active' || currentStatus === 'trialing';
    ...
    console.log(`[Verify Subscription Server] Synchronizing SaaS parameters on businesses for id ${resolvedBusinessId}:`, saasUpdateData);
    await db.from('businesses').update(saasUpdateData).eq('id', resolvedBusinessId);
    ...
    let dbError = null;
    if (existingSub) {
      console.log(`[Verify Subscription Applet] Updating existing subscription for business: ${resolvedBusinessId}`);
      ...
      console.log(`[Verify Subscription Applet] Creating next subscription row for business: ${resolvedBusinessId}`);
    ...
    if (dbError) {
      console.warn('[Verify Subscription Router DB Error (Non-Fatal Fallback)]:', dbError.message);
      console.log('We will delegate database updates to the client frontend where user holds active authenticated session...');
    }

    console.log(`[Verify Subscription Success] Account activated! businessId: ${resolvedBusinessId}, status: ${subStatus}`);
    res.json({
      success: true,
      status: subStatus,
      expiresAt: expiresAt,
...
app.post('/api/stripe/create-portal-session', async (req, res) => {
  try {
    ...
    const calculatedReturnUrl = getRealRedirectUrl(req, `/dashboard?status=portal_closed`, `/dashboard`);
...
const handleStripeWebhook = async (req: any, res: any) => {
  ...
  if (sig) {
    ...
    if (mainSecret) {
      try {
        ...
        console.log('[Webhook:SigVerification] Successfully validated against Main Secret (Corporate channel)');
      } catch (_) {}
    }
    ...
    if (!event && connectSecret) {
      try {
        ...
        console.log('[Webhook:SigVerification] Successfully validated against Connect Platform Secret');
      } catch (err: any) {
        console.warn('[Webhook:SigVerification] Connected secret validation mismatch:', err.message);
      }
    }
  }
  ...
  if (!event) {
    if (mainSecret || connectSecret) {
      res.status(400).send('Webhook signature validation failed on all configured channels.');
      return;
    }
    try {
      event = JSON.parse(rawBody.toString('utf8'));
      console.log('[Webhook:SignatureCheckBypassed] Bypassing security checks - parsing raw JSON event.');
    } catch (_) {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (type === 'subscription' || type === 'pro_subscription') {
        const businessId = session.metadata?.business_id || session.metadata?.businessId;
        const customerId = session.customer as string;
        const subId = session.subscription as string;

        console.log(`[Stripe Webhook checkout.session.completed:subscription] Parsing business_id: ${businessId}, customer: ${customerId}, subscription_id: ${subId}`);

        if (businessId) {
          let subStatus = 'trialing';
          let expiresAt = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString(); // 14-day default

          if (subId) {
            try {
              const stripeSub = await stripe.subscriptions.retrieve(subId);
              subStatus = (stripeSub as any).status === 'trialing' ? 'trialing' : (stripeSub as any).status;
              expiresAt = new Date((stripeSub as any).current_period_end * 1000).toISOString();
            } catch (stripeSubErr: any) {
              console.error('[Webhook Retrieve Sub failed, using fallback metrics]:', stripeSubErr.message);
            }
          }

          console.log(`[Stripe Webhook DEBUG] WEBHOOK RECEIVED: checkout.session.completed, business_id: ${businessId}, subId: ${subId}, status: ${subStatus}`);

          // 1. Link customer ID & subscription details inside Businesses table
          try {
            const { error: bizUpdateErr } = await db.from('businesses').update({
              stripe_customer_id: customerId,
              stripe_subscription_id: subId,
            ...
            if (bizUpdateErr) {
              console.warn('[Webhook Warning] Direct businesses table subscription pillars update failed, attempting standard customer link...');
              await db.from('businesses').update({ stripe_customer_id: customerId }).eq('id', businessId);
            } else {
              console.log(`[Webhook success] Aligned businesses table for id: ${businessId} with customerId and subStatus ${subStatus}`);
              console.log("SUBSCRIPTION ACTIVATED");
            }
          } catch (bizUpdateEx: any) {
            console.warn('[Webhook Exception] Safe update on businesses failed. Continuing with fallback subscription table sync.', bizUpdateEx.message);
            await db.from('businesses').update({ stripe_customer_id: customerId }).eq('id', businessId);
            console.log("SUBSCRIPTION ACTIVATED");
          }
          ...
          if (existingSub) {
            ...
            console.log(`[Webhook SaaS Sync] No subscription found for business ${businessId}. Inserting new subscription row with status '${subStatus}' and stripe_subscription_id '${subId}'...`);
            const { error: insErr } = await db
              .from('subscriptions')
              .insert({
                business_id: businessId,
        ...
        if (bookingId) {
          ...
          const { error: payErr } = await db
            .from('payments')
            .update({ payment_status: 'paid', stripe_payment_intent: session.payment_intent as string })
            .eq('booking_id', bookingId);

          if (bkErr || payErr) {
            console.error('[Webhook Booking Pay Update Err]:', bkErr?.message || payErr?.message);
          } else {
            console.log(`[Webhook Payments Success] Confirmed paid for reservation ID: ${bookingId}`);
          }
          ...
          try {
            ...
            if (bookingRec) {
              const pointsEarned = Math.floor(Number(bookingRec.total_price || 0));
              if (pointsEarned > 0) {
                ...
                if (loyaltyErr) {
                  console.error('[Webhook Loyalty Point Earn DB Err]:', loyaltyErr.message);
                } else {
                  console.log(`[Webhook Loyalty Reward Dispatch] Granted ${pointsEarned} GlamPoints to user: ${bookingRec.customer_id}`);
                }
              }
            }
          } catch (loyaltyDisptchErr: any) {
            console.warn('[Webhook Loyalty Flow Exception]:', loyaltyDisptchErr.message);
          }
        }
      }
    }
    ...
    if (event.type === 'customer.subscription.updated' || event.type === 'customer.subscription.deleted') {
      ...
      let syncedStatus = (liveSubscription as any).status === 'trialing' ? 'trialing' : (liveSubscription as any).status;
      if (event.type === 'customer.subscription.deleted') {
        syncedStatus = 'expired';
      }
      const syncedExpiry = new Date((liveSubscription as any).current_period_end * 1000).toISOString();
      const syncedActive = syncedStatus === 'active' || syncedStatus === 'trialing';

      console.log(`[Stripe Webhook ${event.type}] Parsing customer_id: ${customerId}, subscription_id: ${stripeSubId}, status: ${syncedStatus}`);
      console.log(`[Stripe Webhook DEBUG] SUBSCRIPTION UPDATED: ${stripeSubId}, status: ${syncedStatus}`);

      // Try searching for business ID using either metadata or customer ID
      let businessId = liveSubscription.metadata?.business_id || liveSubscription.metadata?.businessId;
      if (!businessId) {
      ...
      if (businessId) {
        try {
          ...
          console.log(`[Webhook Sync] Aligned businesses subscription info for business: ${businessId}`);
        } catch (bizColUpdateErr: any) {
          console.warn('[Webhook Warning] Failed to update business subscription columns:', bizColUpdateErr.message);
        }
      }
      ...
      if (businessId) {
        ...
        if (existingSub) {
          console.log(`[Webhook SaaS Sync] Updating subscription in db for business ${businessId} to status '${syncedStatus}'`);
          const { error: updErr } = await db
            .from('subscriptions')
            .update({
              status: syncedStatus,
          ...
          console.log(`[Webhook SaaS Sync] Creating subscription record in db for business ${businessId} with status '${syncedStatus}'`);
          const { error: insErr } = await db
            .from('subscriptions')
            .insert({
              business_id: businessId,
      ...
      if (syncErr) {
        ...
        console.log(`[Webhook SaaS Sync Completed] SUBSCRIPTION UPDATED SUCCESSFULLY for Stripe Sub: ${stripeSubId}`);
      }
    }
    ...
    if (event.type === 'invoice.paid' || event.type === 'invoice.payment_failed') {
      ...
      if (stripeSubId) {
        const syncedStatus = event.type === 'invoice.paid' ? 'active' : 'past_due';
        const syncedActive = syncedStatus === 'active';
        console.log(`[Stripe Webhook ${event.type}] Parsing customer_id: ${customerId}, subscription_id: ${stripeSubId}, status: ${syncedStatus}`);

        let businessId = null;
        const { data: bRec1 } = await db.from('businesses').select('id').eq('stripe_customer_id', customerId).maybeSingle();
        if (bRec1) {
          businessId = bRec1.id;
        }
        ...
        if (businessId) {
          try {
            ...
            console.warn('[Webhook Warning] Businesses sub status sync failed on invoice event:', err.message);
          }
        }
        ...
        if (existingSub) {
          await db.from('subscriptions').update({ status: syncedStatus }).eq('id', existingSub.id);
        } else if (businessId) {
    ...
    if (event.type === 'account.updated') {
      ...
      if (bizUpdateErr) {
        console.error('[Webhook Connect Operations Sync Err]:', bizUpdateErr.message);
      } else {
        console.log(`[Webhook Connect Sync Completed] Operational capabilities for account ${stripeAccountId}: charges_enabled=${verifiedCharges}, payouts_enabled=${verifiedPayouts}`);
      }
    }
    ...
    if (event.type === 'payout.paid' || event.type === 'payout.failed') {
      ...
      if (stripeAccountId) {
        ...
        if (business) {
          ...
          const targetStatus = event.type === 'payout.paid' ? 'completed' : 'rejected';
...
async function startServer() {
  if (process.env.NODE_ENV !== 'production') {
    ...
    app.use(express.static(distPath, {
      ...
      setHeaders: (res, filePath) => {
        ...
        } else if (filePath.match(/\.(js|css|svg|png|jpg|jpeg|gif|ico|webp|woff2)$/)) {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Glamzo Full-Stack Server running on http://localhost:${PORT} in ${process.env.NODE_ENV || 'development'} mode`);
  });
}
