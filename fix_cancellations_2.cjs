...
i18n = i18n.replace(/en:\s*\{\s*translation:\s*\{[\s\S]*?"cancellations":\s*\{[\s\S]*?a4_li2":.*?\}/, (match) => {
  return match.replace(/"cancellations":\s*\{[\s\S]*?a4_li2":.*?\}/, cancellationsObj.en.trim());
});
i18n = i18n.replace(/es:\s*\{\s*translation:\s*\{[\s\S]*?"cancellations":\s*\{[\s\S]*?a4_li2":.*?\}/, (match) => {
  return match.replace(/"cancellations":\s*\{[\s\S]*?a4_li2":.*?\}/, cancellationsObj.es.trim());
});
i18n = i18n.replace(/fr:\s*\{\s*translation:\s*\{[\s\S]*?"cancellations":\s*\{[\s\S]*?a4_li2":.*?\}/, (match) => {
  return match.replace(/"cancellations":\s*\{[\s\S]*?a4_li2":.*?\}/, cancellationsObj.fr.trim());
});
