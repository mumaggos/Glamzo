...
import { LayoutDashboard, Calendar, CheckSquare, UsersRound, Users, Scissors, Clock, Tag, Landmark, Globe, MessageSquare, Smartphone, Settings, LogOut, Sparkles, X, Menu, Bell } from "lucide-react";
...
export default function PartnerLayout() {
  ...
  if (authLoading || !business) return <div className="min-h-screen flex items-center justify-center bg-slate-50"><div className="animate-spin w-8 h-8 border-4 border-purple-500 border-t-transparent rounded-full" /></div>;
  ...
  return (
    <div id="partner-terminal-layout" className="min-h-screen bg-[#F8F9FC] text-slate-800 flex font-sans select-none overflow-hidden h-screen relative">
              ...
              {navItems.map((tab) => {
                ...
                return (
                  <Link key={tab.id} to={tab.path} className={`w-full flex items-center px-4 py-3 text-sm rounded-2xl font-bold transition-all ${isActive ? "bg-purple-600 text-white shadow-md" : "text-slate-600 hover:bg-slate-50"}`}>
                    <Icon className="w-4 h-4 mr-3 shrink-0" /> {tab.label}
                  </Link>
                );
              })}
            </nav>
            ...
            {navItems.map((tab) => {
              ...
              return (
                <Link key={tab.id} to={tab.path} className={`w-full flex items-center justify-between px-4 py-3 text-xs rounded-2xl font-bold transition-all ${isActive ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}>
                  <div className="flex items-center gap-3"><Icon className="w-4 h-4 shrink-0" /> <span>{tab.label}</span></div>
                </Link>
              );
            })}
          </nav>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        {/* Header Topo - Sem z-index para permitir popups */}
        <header className="h-16 px-4 sm:px-8 flex items-center justify-between shrink-0 bg-transparent pt-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="relative p-2.5 bg-white text-slate-400 hover:text-purple-600 transition-colors shadow-sm border border-slate-200 rounded-full cursor-pointer">
        ...
        {/* Scroll Content - Sem z-index */}
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-4 pb-24 md:pb-6">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t pb-safe pt-2 px-6 flex justify-between items-center z-[50] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
