...
export default function PartnerLayout() {
  ...
  const [notifications, setNotifications] = useState(() => {
    ...
    if (sessionStorage.getItem('dismissed_welcome') !== 'true') {
      notifs.push({ id: 1, title: t('partner.systemUpdatedTitle'), desc: t('partner.systemUpdatedDesc'), time: t('partner.timeNow') });
    }
    return notifs;
  });
  ...
  const navItems = [
    { id: "overview", label: t('partner.tabOverview'), icon: LayoutDashboard, path: "/partner/dashboard/overview" },
    { id: "agenda", label: t('partner.tabAgenda'), icon: Calendar, path: "/partner/dashboard/agenda" },
    { id: "reservas", label: t('partner.tabBookings'), icon: CheckSquare, path: "/partner/dashboard/reservas" },
    { id: "clientes", label: t('partner.tabClients'), icon: UsersRound, path: "/partner/dashboard/clientes" },
    { id: "equipa", label: t('partner.tabStaff'), icon: Users, path: "/partner/dashboard/equipa" },
    { id: "servicos", label: t('partner.tabServices'), icon: Scissors, path: "/partner/dashboard/servicos" },
    { id: "horarios", label: t('partner.tabSchedule'), icon: Clock, path: "/partner/dashboard/horarios" },
    ...
    { id: "subscricao", label: t('partner.tabPlan'), icon: CreditCard, path: "/partner/dashboard/subscricao" },
    ...
    <div id="partner-terminal-layout" className="h-[100dvh] bg-[#F8F9FC] text-slate-800 flex flex-col lg:flex-row font-sans select-none overflow-hidden relative">
      ...
      <div className={`fixed inset-0 z-[100] flex lg:hidden transition-opacity duration-300 ${isMobileSidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`relative flex flex-col w-72 h-full bg-white border-r shadow-2xl z-10 transform transition-transform duration-300 ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex items-center justify-between p-5 border-b shrink-0 bg-slate-50">
            <div className="flex items-center gap-3"><GlamzoLogo size={28} glow={false} /><span className="font-extrabold text-sm">{t('partner.menu')}</span></div>
      ...
      <aside className="hidden lg:flex w-[260px] border-r border-slate-200 bg-white flex-col shrink-0 h-full z-20 shadow-sm">
         <button onClick={async () => { await signOut(); navigate("/"); }} className="h-20 border-b border-slate-100 flex items-center px-6 gap-3 w-full text-left hover:bg-slate-50">
            ...
            <div><span className="font-black text-sm block">Glamzo</span><span className="text-[10px] font-bold text-slate-500">{t('partner.eliteWorkspace')}</span></div>
          </button>
          <div className="p-4 mx-4 my-4 bg-slate-50 border border-slate-100 rounded-2xl">
            ...
            <div className="flex items-center gap-1.5 mt-1"><span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" /><span className="text-[10px] text-slate-500 font-bold">{t('partner.activeSystem')}</span></div>
          </div>
        ...
        <div className="relative z-[99999] h-16 px-4 sm:px-8 flex items-center justify-between shrink-0 bg-white/50 backdrop-blur-md pt-4 border-b border-slate-100/50">
          ...
          <div className="flex items-center gap-3 relative">
              ...
              <>
                ...
                <div className="absolute right-0 top-14 w-80 bg-white border border-slate-200 shadow-2xl rounded-3xl z-[99999] overflow-hidden animate-in fade-in slide-in-from-top-4">
                  <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                    <h4 className="font-extrabold text-slate-900">{t('partner.notifications')}</h4>
                    <span className="text-[10px] font-bold bg-rose-100 text-rose-600 px-2 py-0.5 rounded-full">{notifications.length} {t('partner.newNotifications')}</span>
                  </div>
                  <div className="p-2 space-y-1">
                    {notifications.length === 0 ? (
                      <div className="py-6 text-center text-xs text-slate-500 font-medium">{t('partner.noNotifications')}</div>
                    ) : (
                      notifications.map(n => (
            ...
            <div className="flex bg-white shadow-sm border border-slate-200 px-3 py-2.5 rounded-full items-center gap-2">
               ...
               <span className="text-[10px] font-bold text-slate-700 uppercase tracking-widest hidden sm:inline">{t('partner.online')}</span>
            </div>
          </div>
        </div>
