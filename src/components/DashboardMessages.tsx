...
export default function DashboardMessages({ businessId }: { businessId: string }) {
  ...
  return (
    <div className="flex bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden h-[600px] shadow-2xl relative">
      <div className={`w-full md:w-1/3 border-r border-slate-800 flex flex-col ${selectedSession ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-4 border-b border-slate-800 bg-slate-950/40">
          <h3 className="font-extrabold text-white mb-3">Mensagens</h3>
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-slate-900 border border-slate-700 text-slate-200 pl-9 pr-3 py-2 rounded-xl text-xs focus:border-purple-500 outline-none"
            />
          </div>
        </div>
            ...
            activeSessions.map(sess => (
              ...
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="font-bold text-slate-100 text-[11px] truncate">{sess.customer_name}</span>
                ...
                <span className="block text-[10px] text-slate-400 truncate w-full">{sess.last_message || 'Início da conversa'}</span>
              </button>
            ))
          )}
        </div>
      ...
      <div className={`w-full md:w-2/3 flex flex-col bg-[#0a0f1d] ${!selectedSession ? 'hidden md:flex' : 'flex'}`}>
        {!selectedSession ? (
          <div className="flex-1 flex flex-col items-center justify-center text-slate-500 p-6">
            ...
            <p className="text-sm font-bold text-slate-300">Selecione uma conversa</p>
            <p className="text-xs mt-1">Dê suporte rápido aos seus clientes diretamente pelo terminal.</p>
            
            <div className="mt-8 p-4 bg-slate-900/80 border border-slate-800 rounded-2xl max-w-sm text-left">
              <div className="flex items-start gap-3">
                <Clock className="w-5 h-5 text-purple-400 mt-0.5" />
                <div>
                  <h4 className="text-[11px] font-bold text-white uppercase tracking-wider mb-1">Estatísticas de Resposta</h4>
                  <p className="text-[10px] text-slate-400">Tempo médio de resposta: <span className="font-bold text-emerald-400">~ 5 minutos</span></p>
          ...
          <>
            ...
            <div className="p-4 border-b border-slate-800 flex items-center gap-3 bg-slate-950/40">
              <button 
                ...
                className="md:hidden p-1.5 bg-slate-800 rounded-lg text-slate-300 hover:text-white"
              >
                <ArrowLeft className="w-4 h-4" />
              </button>
              <div>
                <span className="block font-black text-white text-xs uppercase tracking-tight">{selectedSession.customer_name}</span>
            ...
            <div className="px-4 py-2 bg-slate-900 border-t border-slate-800 text-[9px] text-slate-500 text-center font-mono">
            ...
            <form onSubmit={handleSendMessage} className="p-4 bg-slate-950/40 border-t border-slate-800 flex gap-2">
              <input 
                ...
                className="flex-1 bg-slate-900 border border-slate-700 text-white text-xs rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500 placeholder:text-slate-500"
              />
              <button
                type="submit"
                disabled={!chatInput.trim()}
                className="bg-purple-600 hover:bg-purple-500 text-white px-4 rounded-xl disabled:bg-slate-800 disabled:text-slate-500 cursor-pointer transition-colors"
