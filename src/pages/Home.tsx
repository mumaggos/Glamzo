...
const getCustomMarkerIcon = (rating: number) => {
  ...
  return `data:image/svg+xml;utf-8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="56" height="42" viewBox="0 0 56 42"><g filter="drop-shadow(0px 4px 6px rgba(0,0,0,0.3))"><path d="M 8 2 L 48 2 C 51.3 2 54 4.7 54 8 L 54 22 C 54 25.3 51.3 28 48 28 L 34 28 L 28 38 L 22 28 L 8 28 C 4.7 28 2 25.3 2 22 L 2 8 C 2 4.7 4.7 2 8 2 Z" fill="#9333ea" stroke="#ffffff" stroke-width="1.5" /><text x="28" y="19" fill="#ffffff" font-size="12px" font-family="Outfit" font-weight="900" text-anchor="middle">${ratingText}</text></g></svg>`)}`;
};
        ...
        <div className="h-[500px] rounded-3xl overflow-hidden bg-slate-100">
          {mapLoaded && API_KEY ? (
             <APIProvider apiKey={API_KEY}>
               <Map defaultCenter={{ lat: 39.3999, lng: -8.2245 }} defaultZoom={6} disableDefaultUI styles={mapStyles}>
                 {businesses.map((b) => (
                   <Marker key={b.id} position={{ lat: b.latitude, lng: b.longitude }} icon={{ url: getCustomMarkerIcon(5), anchor: { x: 29, y: 32 } }} />
                 ))}
               </Map>
             </APIProvider>
          ) : (
