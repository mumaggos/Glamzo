...
import { PremiumCalendarTab } from '../components/dashboard/PremiumCalendarTab';
...
export default function Dashboard() {
  ...
  const handleOpenBillingPortal = async () => {
    ...
    try {
      ...
      console.warn(
        "Falha ao abrir Billing Portal, executando checkout alternativo...",
        err,
      );
    }
  ...
  const handleSubscribePro = async (planName: "PRO" | "TERMINAL" = "PRO") => {
    ...
    try {
      notifyTerminal(
        "💳 Iniciar Checkout",
        `A preparar o seu Glamzo Pay Checkout do Plano ${planName}...`,
      );
      ...
      if (response.ok && resData?.success && resData?.url) {
        ...
        const errorMsg =
          resData?.error || "Falha ao gerar o link do Glamzo Pay Checkout.";
      ...
      alert(
        `Erro técnico ao processar pagamento: ${err.message || "Tente novamente."}`,
      );
    }
  };
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      notifyTerminal(
        "🚫 Cancelar Subscrição",
        "A comunicar desativação e paragem ao Glamzo Pay...",
      );
      ...
      if (response.ok && resData?.success) {
        notifyTerminal(
          "✔ Desativado",
          "A sua subscrição foi desativada com sucesso.",
        );
        setGlobalSuccess(
          `A subscrição ${business?.selected_plan === "app_tablet" ? "PRO Terminal" : "Glamzo PRO"} foi desativada. O seu salão foi ocultado do público.`,
        );
        ...
        const errorMsg =
          resData?.error || "Ocorreu um erro ao cancelar no servidor.";
  ...
  const handleSubmitPayoutRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      setPayoutSuccess(
        "Operação efetuada. Pedido de transferência de fundos real registado com estado 'Pendente'.",
      );
      await loadTerminalData();
    } catch (err: any) {
      setGlobalError(
        err.message || "Falha ao processar pedido de transferência.",
      );
    }
  };
  ...
  const handleDeleteService = async (id: string) => {
    if (
      !window.confirm("Pretende realmente eliminar este serviço da plataforma?")
    )
      return;
    try {
  ...
  const handleDeleteStaff = async (id: string) => {
    if (!window.confirm("Pretende apagar o registo deste profissional?"))
      return;
    try {
  ...
  const handleCopyHoursToAll = async (sourceWeekday: number) => {
    ...
    try {
      ...
      setGlobalError(
        "Não foi possível copiar o horário para os restantes dias.",
      );
    }
  };
  ...
  const handleCreateCoupon = async (e: React.FormEvent) => {
    ...
    if (!business) return;
    try {
      const pct = couponForm.discount_percent
        ? parseFloat(couponForm.discount_percent)
        : null;
      const val = couponForm.discount_value
        ? parseFloat(couponForm.discount_value)
        : null;

      if (!pct && !val) {
        setGlobalError(
          "Forneça pelo menos uma percentagem (%) ou valor fixo (€) de desconto.",
        );
        return;
      }
      ...
      try {
        ...
        if (dbErr) {
          console.warn(
            "Table business_coupons structure might be setup-pending on Supabase, falling back to cached local storage:",
            dbErr.message,
          );
        }
      } catch (dbErr: any) {
        console.warn(
          "Supabase network error, writing to offline-first local cache:",
          dbErr.message,
        );
      }

      // 2. Always persist coupon to client-side localStorage cache for immediate booking application
      const currentLocals = JSON.parse(
        localStorage.getItem("glamzo_coupons") || "[]",
      );
      // Exclude duplicates on local cache
      const updatedLocals = currentLocals.filter(
        (lc: any) =>
          !(lc.business_id === business.id && lc.code === newLocalCoupon.code),
      );
  ...
  const handleToggleCoupon = async (id: string, currentStatus: boolean) => {
    try {
      ...
      const updatedLocals = currentLocals.map((c: any) => {
        if (
          c.id === id ||
          (c.code && id.startsWith("coupon-") && c.id === id)
        ) {
  ...
  const handleDeleteCoupon = async (id: string) => {
    ...
    try {
      // 1. Try online
      try {
        await supabase.from("business_coupons").delete().eq("id", id);
      } catch (_) {}
  ...
  const uniqueClientsMap = new Map<
    string,
    { name: string; email: string; visits: number; spend: number }
  >();
  bookings.forEach((bk) => {
    ...
    const name =
      ...
      "Cliente Particular";
    const email =
      bk.customer?.email || bk.customer_profile?.email || "geral@cliente.pt";
    const visitsInc = bk.booking_status === "completed" ? 1 : 0;
    const spendInc =
      bk.booking_status === "completed" ? Number(bk.total_price) : 0;
          ...
          {loading ? (
            ...
            <>
              ...
              {activeTab === "dashboard" && business && (
                <OverviewTab business={business} onNavigate={setActiveTab} />
              )}
              {activeTab === "agenda" && business && (
                <PremiumCalendarTab business={business} staff={staff} bookings={bookings} />
              )}
              {activeTab === "reservas" && (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-5">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Consulte e gira todos os agendamentos feitos pelos seus
                        clientes e equipa num único ecrã.
                      </p>
                    </div>
                  </div>
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead className="bg-white border-b border-slate-100 text-[10px] font-bold text-slate-500 text-slate-500 uppercase tracking-widest leading-none">
                          <tr>
                            ...
                            <th className="py-4.5 px-4 text-right font-mono">
                              Preço
                            </th>
                        ...
                        <tbody className="divide-y divide-slate-900 text-xs">
                          {bookings
                            .filter((b) => {
                              ...
                              const matchesSearch =
                                custName.includes(terms) ||
                                sName.includes(terms);
                              ...
                              >
                                ...
                                <td className="py-4 px-4">
                                  <span className="font-bold text-slate-700">
                                    {bk.service?.name || "Serviço Premium"}
                                  </span>
                                  <span className="block text-[10px] text-slate-550 text-slate-500 font-mono mt-0.5">
                                ...
                                <td className="py-4 px-4 text-center">
                                  <span
                                    className={
                                      "inline-block px-2.5 py-0.5 border rounded-full text-[9px] font-bold font-mono uppercase " +
                          ...
                          {bookings.length === 0 && (
                            <tr>
                              <td
                                colSpan={7}
                                className="text-center py-16 text-slate-500 text-xs font-mono space-y-2"
                              >
                                <AlertCircle className="w-10 h-10 text-slate-600 mx-auto" />
                                <p className="text-slate-500">
                                  Ainda não recebeu marcações. Os agendamentos
                ...
                <div id="view-servicos" className="space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-5">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Configuração dos seus procedimentos estéticos com preços
                        e durações reais.
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {services.map((sv) => (
                      <div
                        key={sv.id}
                        className="bg-slate-50 border border-slate-100 rounded-3xl p-5 hover:border-slate-200 transition-all space-y-4 flex flex-col justify-between"
                      >
                        <div>
                          <p className="text-[9px] font-mono font-bold uppercase tracking-widest text-rose-400">
                            {sv.category?.name || "Geral"}
                          </p>
                          <h4 className="text-base font-black text-slate-900 mt-1">
                            {sv.name}
                          </h4>
                          <p className="text-xs text-slate-500 mt-2 leading-relaxed line-clamp-2">
                            {sv.description || "Sem descrição cadastrada."}
                          </p>
                        </div>
                        ...
                        <div className="border-t border-slate-200/80 pt-4 flex items-center justify-between text-xs mt-auto">
                          <div className="font-mono">
                            ...
                            <span className="text-slate-500 text-slate-500 font-bold ml-1.5 font-sans">
                      ...
                      <div className="col-span-1 md:col-span-3 text-center py-16 bg-slate-50 border border-dashed border-slate-200 rounded-3xl text-sm font-mono text-slate-500 space-y-3">
                        ...
                        <div>
                          <p className="font-bold text-slate-600">
                            Sem serviços criados.
                          </p>
                          <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                            Crie os seus serviços (como cortes, colorações ou
                            unhas) para que apareçam no seu salão online.
                          </p>
                        </div>
                      </div>
                    )}
                ...
                <div id="view-equipa" className="space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-5">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Gira as escalas, competências e horários de folga dos
                        membros da equipa.
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {staff.map((st) => (
                      <div
                        key={st.id}
                        className="bg-slate-50 border border-slate-100 rounded-3xl p-5 hover:border-slate-200 text-center flex flex-col items-center gap-3"
                      >
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-slate-500 font-bold uppercase mt-0.5 block">
                            {st.role_title || "Artista Cabelo"}
                          </span>
                        </div>

                        <span
                          className={`inline-block px-2 py-0.5 rounded-full text-[9px] font-mono font-bold leading-none ${
                            st.is_active
                    ...
                    {staff.length === 0 && (
                      <div className="col-span-1 md:col-span-4 text-center py-16 bg-slate-50 border border-dashed border-slate-200 rounded-3xl text-sm font-mono text-slate-500 space-y-3">
                        ...
                        <div>
                          <p className="font-bold text-slate-600">
                            Ainda não tem profissionais adicionados.
                          </p>
                          <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                            Adicione a sua equipa para que as suas clientes
                            possam agendar diretamente com os profissionais
                            favoritos.
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="fixed inset-0 bg-white/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                      <div className="bg-slate-50 border border-slate-200 rounded-3xl w-full max-w-sm p-6 sm:p-8 space-y-6 text-slate-800">
                        <div className="flex justify-between items-center border-b border-slate-200 pb-4">
                          <h4 className="font-extrabold text-base text-slate-900">
                            {editingStaff
                              ? "Editar Detalhes do Profissional"
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {["D", "S", "T", "Q", "Q", "S", "S"].map(
                                (letter, dayIdx) => {
                                  const offDaysArr = staffForm.off_days
                                    ? staffForm.off_days
                                        .split(",")
                                        .map((item) => item.trim())
                                    : [];
                                  const isSelected = offDaysArr.includes(
                                    String(dayIdx),
                                  );
                                  return (
                                    <button
                            ...
                            <p className="text-[10px] text-slate-500 font-sans mt-1 leading-normal">
                              O profissional selecionado ficará indisponível
                              para marcação automática nestes dias.
                            </p>
                          </div>
                          ...
                          <div className="flex items-center gap-2 pt-2">
                            ...
                            />
                            <label
                              htmlFor="staff-active-check"
                              className="text-slate-600 font-bold"
                            >
                              Activo na Escala Diária
                            </label>
                          </div>
                    ...
                    >
                      <div className="border-b border-slate-100 pb-5 text-left">
                        ...
                        <p className="text-xs text-slate-500 text-slate-500 mt-1 leading-normal">
                          Defina de forma simples as horas de funcionamento
                          reais e dias de encerramento do salão. A sua agenda
                          online atualizará instantaneamente no marketplace da
                          Glamzo.
                        </p>
                      </div>
                      ...
                      <div className="bg-slate-50 border border-slate-100/50 rounded-3xl p-5 sm:p-7 space-y-4">
                        <div className="space-y-3.5">
                          {weekdaysOrdered.map(({ name, idx }) => {
                            ...
                            const currentOpen = match
                              ? match.open_time
                              : "09:00";
                            const currentClose = match
                              ? match.close_time
                              : "19:00";
                            ...
                            return (
                              <div
                                key={idx}
                                className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 bg-white rounded-2xl border border-slate-100/80 hover:border-slate-200 transition-all text-left"
                              >
                                <div className="flex items-center gap-3 shrink-0">
                                  <span
                                    className={`w-2.5 h-2.5 rounded-full ${isClosed ? "bg-rose-500" : "bg-emerald-500 animate-pulse"}`}
                                  />
                                  <span className="font-extrabold text-sm text-slate-900 w-28 uppercase tracking-wide font-mono">
                                    {name}
                                  </span>
                                </div>
                                ...
                                <div className="flex flex-wrap items-center gap-3">
                                  ...
                                  <span className="text-slate-600 font-extrabold text-[10px] font-mono uppercase">
                                    até
                                  </span>
              ...
              {activeTab === "clientes" && (
                <div id="view-clientes" className="space-y-6">
                  <div className="border-b border-slate-100 pb-5">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Histórico automático de visitas e gastos individuais de
                      cada cliente.
                    </p>
                  </div>
                      ...
                      <table className="w-full text-left">
                        <thead className="bg-white text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none border-b border-slate-100">
                          <tr>
                            ...
                            <th className="py-4.5 px-4 text-center font-mono">
                              Visitas Concluídas
                            </th>
                            <th className="py-4.5 px-4 text-right font-mono">
                              Gasto Bruto
                        ...
                        <tbody className="divide-y divide-slate-900 text-xs">
                          {clientsList.map((client, idx) => {
                            ...
                            let tierBadge =
                              "bg-white border-slate-200 text-slate-500 text-slate-500";
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="py-4.5 px-6 text-center">
                                  <span
                                    className={`inline-block px-2.5 py-0.5 border rounded-full text-[9px] font-mono font-bold uppercase tracking-tight ${tierBadge}`}
                                  >
                                    {loyaltyTier}
                                  </span>
                                </td>
                          ...
                          {clientsList.length === 0 && (
                            <tr>
                              <td
                                colSpan={6}
                                className="text-center py-16 text-slate-500 text-xs font-mono"
                              >
                                <UsersRound className="w-12 h-12 text-slate-700 mx-auto mb-2" />
                                <p>
                                  Ainda não tem clientes registados no seu
                                  histórico.
                                </p>
                              </td>
                            </tr>
              ...
              {activeTab === "financeiro" && (
                ...
                >
                  <div className="border-b border-slate-100 pb-5 text-left">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Acompanhe a sua subscrição Glamzo Pro, consulte as suas
                      faturas reais e verifique o estado do Glamzo Pay Connect.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-left border-l-2 border-l-purple-500">
                      ...
                      <span className="text-base sm:text-lg font-black text-slate-900 mt-1.5 block font-mono">
                        {totalVolumeBruto.toFixed(2)} €
                      </span>
                    </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-left border-l-2 border-l-orange-500">
                      ...
                      <span className="text-base sm:text-lg font-black text-rose-350 mt-1.5 block font-mono">
                        -{totalComissoesRetidas.toFixed(2)} €
                      </span>
                    </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-left border-l-2 border-l-emerald-500">
                      ...
                      <span className="text-base sm:text-lg font-black text-emerald-400 mt-1.5 block font-mono">
                        {totalReceivedVolume.toFixed(2)} €
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 sm:p-6 space-y-4 text-left font-sans">
                    <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                      <div>
                        <h4 className="font-extrabold text-sm text-slate-900">
                          Transações & Faturação de Clientes
                        </h4>
                        <p className="text-[10px] text-slate-500 text-slate-500 mt-0.5 font-medium">
                          Histórico de liquidações e processos de faturação
                          automática integrada via Glamzo Pay.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-white/40 p-4 border border-slate-200 rounded-2xl text-[11px] leading-relaxed text-slate-500 space-y-2">
                      <p className="text-slate-900 font-extrabold flex items-center gap-1.5">
                        ...
                        Faturação 100% Automatizada e Integrada
                      </p>
                      <p>
                        No ambiente de produção da Glamzo,{" "}
                        <span className="text-slate-700 font-bold">
                          não necessita de preencher SAF-T manuais nem de emitir
                          PDFs temporários
                        </span>
                        .
                      </p>
                      <p>
                        No exato momento em que um cliente final efetua o
                        pagamento de uma marcação via Glamzo Pay Checkout, a{" "}
                        <span className="text-slate-900">
                          plataforma emite e envia automaticamente o recibo
                          oficial e termo de faturação correspondente
                        </span>{" "}
                        diretamente para o email do cliente. O histórico fiscal
                        fica arquivado na sua conta bancária conectada e no
                        e-mail do cliente, cumprindo todas as diretrizes
                        regulatórias e fiscais aplicáveis.
                      </p>
                    </div>
                    ...
                    <div className="space-y-3 max-h-[350px] overflow-y-auto scrollbar-thin">
                      {ledgers.map((item, idx) => {
                        const originalPrice = Number(
                          item.amount_total || item.amount || 0,
                        );
                        const platformFee = Number(item.glamzo_fee || 0);
                        const merchantProfit = Number(
                          item.business_amount || 0,
                        );
                        ...
                        return (
                          <div
                            key={idx}
                            className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 gap-3 text-xs"
                          >
                            <div className="space-y-1">
              ...
              {activeTab === "campanhas" && (
                ...
                >
                  <div className="border-b border-slate-100 pb-5">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Promova promoções, atraia clientes em dias de menor
                      afluência e defina cupões promocionais reais.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start text-left">
                    ...
                    <div className="bg-slate-50 border border-slate-100/50 rounded-3xl p-5 sm:p-6 space-y-4">
                      ...
                      <p className="text-[10px] text-slate-500 leading-normal">
                        Os clientes introduzem estes códigos dinamicamente no
                        checkout da Glamzo para aplicar descontos reais.
                      </p>

                      <div className="space-y-3.5 max-h-[380px] overflow-y-auto scrollbar-thin">
                        {coupons.map((cp, idx) => {
                          const hasPct =
                            cp.discount_percent !== null &&
                            cp.discount_percent !== undefined;
                          const hasVal =
                            cp.discount_value !== null &&
                            cp.discount_value !== undefined;
                          ...
                          return (
                            <div
                              key={cp.id || idx}
                              className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 gap-3 text-xs"
                            >
                              <div>
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <button
                                    onClick={() =>
                                      handleToggleCoupon(cp.id, cp.is_active)
                                    }
                                ...
                                <p className="text-[10px] text-slate-550 text-slate-500 leading-normal font-medium">
                                  {expiryStr}
                                </p>
                              </div>
                        ...
                        {coupons.length === 0 && (
                          <div className="text-center py-10 border border-dashed border-slate-200 rounded-2xl bg-white/20">
                            <p className="text-xs text-slate-500 font-bold">
                              Sem cupões promocionais criados.
                            </p>
                            <p className="text-[10px] text-slate-500 mt-1">
                              Crie códigos de desconto no formulário ao lado
                              para oferecer promoções especiais aos seus
                              clientes.
                            </p>
                          </div>
                        )}
                    ...
                    <div className="bg-slate-50 border border-slate-100/50 rounded-3xl p-5 sm:p-6 space-y-4">
                      ...
                      <p className="text-[10px] text-slate-500 leading-normal">
                        Configure um novo código promocional específico para
                        atrair e fidelizar a sua clientela.
                      </p>
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-3xl p-6 sm:p-8 space-y-4">
                    ...
                    <p className="text-[11px] text-slate-500 leading-normal font-sans">
                      Selecione um lote de clientes com mais de 3 meses de
                      inatividade e dispare uma notificação automática apelando
                      ao retorno.
                    </p>
                    <button
                      onClick={() => {
              ...
              {/* ==================================================== */}
              {activeTab === "loja" && business && (
                <StoreProfileTab business={business} />
              )}
              {activeTab === "terminal" && (
                <div
                  id="view-terminal"
