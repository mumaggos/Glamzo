...
import { PORTUGAL_GEO, getCoordinatesForCity, calculateDistanceInKm } from '../utils/geoData';
...
export default function Explore() {
  ...
  const fetchExploreData = async () => {
    ...
    try {
      ...
      loadedBiz.forEach((b) => {
        ...
        const active = isPromoted && (!endsAt || new Date(endsAt).getTime() > now);
      ...
      setErrorMsg('Falha ao descarregar base de dados de salões. Volte a tentar mais tarde.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleNearMeToggle = () => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.warn('Geolocation failed or denied. Falling back to default region.', err);
          // Fallback Lisbon coordinates to allow testing proximity math seamlessly!
          setUserCoords({ latitude: 38.7223, longitude: -9.1393 });
          setUseNearMe(true);
          setGeoLocating(false);
          setErrorMsg('Não foi possível adquirir a sua localização exata automaticamente. Ativamos uma localização de teste por aproximação (Lisboa) para demonstrar as distâncias.');
        },
  ...
  const filteredBusinesses = processedBusinesses.filter((b) => {
    ...
    if (!useNearMe) { // Ignore district checks if radius distance search is active
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-5">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-2 max-w-xl font-normal leading-relaxed">
              Marketplace premium de estética com faturamento integrado, estimativa de inteligência geográfica e marcações em tempo real.
            </p>
          </div>
        ...
        {selectedCategory !== 'All' && SUBCATEGORIES_BY_MAIN[selectedCategory] && (
          <div className="mb-8 p-4 bg-white border border-slate-200/60 rounded-2xl animate-fade-in shadow-sm">
            <span className="block text-[10px] font-bold uppercase text-purple-600 tracking-wider mb-2.5">Subcategorias de {selectedCategory}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="hidden lg:block bg-white p-6 border border-slate-205 rounded-2xl space-y-7 self-start shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              ...
              {(searchQuery || selectedCategory !== 'All' || selectedDistrict !== 'All' || useNearMe || minRating > 0 || priceLevel !== 'All' || filterHomeService || filterInstantBooking || filterPremiumPartner || filterAvailableToday) && (
                <button
              ...
              {geoLocating ? (
                ...
                <div className="space-y-3.5 p-3.5 bg-purple-50/50 rounded-2xl border border-purple-100 transition-all">
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      onClick={() => { setUseNearMe(false); setUserCoords(null); }}
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {Object.keys(PORTUGAL_GEO).sort().map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {selectedDistrict !== 'All' && (PORTUGAL_GEO[selectedDistrict] || []).map(c => (
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <div className="lg:hidden flex gap-2">
              ...
              >
                ...
                <span>Filtrar Estabelecimentos ({sortedBusinesses.length})</span>
              </button>
            </div>
            ...
            {loading ? (
                  ...
                  {paginatedBusinesses.map((b) => (
                    ...
                    >
                          ...
                          <div className="absolute top-4 right-4 flex flex-col items-end gap-1.5">
                            ...
                            {(b.rating ?? 4.5) > 4.7 && (
                              <div className="bg-amber-55 text-amber-800 bg-amber-50 font-bold shadow-sm px-2.5 py-1 rounded-full flex items-center gap-1 border border-amber-200 text-[9px] select-none">
                                ...
                                <span className="uppercase tracking-wider">Premium Partner</span>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="pt-4">
                            ...
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider border ${
                                ...
                                <span className={`w-1.5 h-1.5 rounded-full ${b.isOpenNow ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`} />
                            ...
                            <h3 className="text-lg font-bold text-slate-950 tracking-tight leading-snug group-hover:text-purple-600 transition-colors uppercase flex items-center gap-2 flex-wrap font-display">
                              <span>{b.name}</span>
                              {b.is_premium && (
                                <span className="bg-gradient-to-r from-amber-500 to-amber-600 text-white text-[9px] font-black uppercase px-2 py-0.5 rounded-full shadow-sm flex items-center gap-0.5 select-none shrink-0" title="Parceiro de destaque com serviços validados">
                                  👑 Premium
                                </span>
                              )}
                            </h3>
                            ...
                            <div className="flex items-center gap-1 mt-1 font-mono text-xs">
                              {b.reviewsCount > 0 ? (
                                <>
                                  ...
                                  <span className="font-extrabold text-slate-800">{(b.rating).toFixed(1)}</span>
                              ...
                              <div className="mt-4 pt-3 border-t border-slate-100">
                                ...
                                <div className="space-y-1.5">
                                  {services
                                    ...
                                    .slice(0, 3)
                                    .map(s => (
                                      <div key={s.id} className="flex justify-between items-center text-xs text-slate-650 font-medium">
                                        <span className="truncate pr-2">{s.name}</span>
                                        <span className="font-mono font-bold text-purple-600 bg-purple-50 px-2 py-0.5 rounded border border-purple-100/50 text-[10px] shrink-0">{s.price}€</span>
                                      </div>
                                    ))}
                      ...
                      <div className="px-6 py-5 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
                        ...
                        <div
                          className="flex items-center gap-1.5 text-xs font-bold text-white bg-purple-600 hover:bg-purple-705 px-5 py-3 rounded-xl transition-all shadow-sm cursor-pointer"
                        >
              ...
              <div id="no-explore-results" className="bg-white p-16 sm:p-24 border border-slate-150 rounded-2xl text-center flex flex-col items-center shadow-xs">
            ...
            <div className="w-screen max-w-md bg-white border-l border-slate-200 flex flex-col p-6 shadow-2xl relative transition-transform duration-200 ease-out translate-x-0 transform-gpu text-slate-800">
              ...
              <div className="flex-1 overflow-y-auto pr-1 space-y-6">
                ...
                <div className="p-4 bg-purple-50/50 rounded-2xl border border-purple-100 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="block text-[10px] font-bold text-slate-700 uppercase tracking-wider">Calculadora de GPS</span>
                    <button
                      type="button"
                      onClick={() => {
                        handleNearMeToggle();
                      ...
                      className={`text-[10px] font-bold px-3 py-1 rounded-full border transition-all ${
                        useNearMe ? 'bg-purple-600 border-purple-600 text-white' : 'bg-white border-purple-200 text-purple-700 shadow-sm'
                      }`}
                    >
                      ...
                      {[1, 3, 5, 10, 25, 50].map((r) => (
                        <button
                          ...
                          className={`py-1.5 text-[10px] font-bold rounded-lg border text-center transition-all ${
                            nearMeRadius === r ? 'bg-purple-600 border-purple-550 text-white' : 'bg-white border-slate-200 text-slate-600'
                          }`}
                        >
                          {r} km
                        </button>
                    ...
                    {[0, 4, 4.5, 4.8].map((score) => (
                      <button
                        ...
                        className={`py-2 text-[10px] font-bold rounded-lg border text-center transition-all ${
                          minRating === score ? 'bg-purple-600 border-purple-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-600'
                        }`}
                      >
                        {score === 0 ? 'Qualquer' : `${score} ★`}
                      </button>
