...
import { User, Mail, Calendar, Upload, Loader2, Link, Save, CheckCircle, ShieldAlert, Gift, Sparkles, Copy, Check, Star, MessageSquare, AlertCircle, X, Shield } from 'lucide-react';
...
export default function Account() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateProfile(fullName, avatarUrl || null);
  ...
  return (
    <div id="account-view" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 font-sans">
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-500 mt-1">Gerencie seus dados pessoais, role e login unificado do Supabase.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-6 border border-slate-100 rounded-2xl shadow-sm text-center flex flex-col items-center">
          ...
          <div className="relative group w-24 h-24 mb-4">
            {avatarUrl ? (
              <img
                src={avatarUrl}
                alt="Avatar"
                className="w-24 h-24 rounded-full object-cover border-2 border-rose-100"
          ...
          <div className="w-full border-t border-slate-100 pt-4 text-left space-y-3.5">
            ...
            <div>
              ...
              <span className="bg-emerald-50 text-emerald-800 text-[10px] font-mono font-bold uppercase px-2.5 py-1 rounded-full border border-emerald-100 inline-block mt-1">
            ...
            <div>
              ...
              <span className="bg-rose-50 text-rose-700 text-[10px] font-mono font-black uppercase px-2.5 py-1 rounded-full border border-rose-100 inline-block mt-1 animate-pulse">
        ...
        <div className="md:col-span-2 bg-white p-6 sm:p-8 border border-slate-100 rounded-2xl shadow-sm">
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
              <label htmlFor="edit-avatarurl-field" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
              ...
              <p className="text-[10px] text-slate-400 mt-1 leading-normal font-sans">
                Se preferir não usar o upload do Storage, forneça uma URL direta de imagem (por exemplo, do Imgur ou Unsplash).
              </p>
            </div>
      ...
      {/* Bookings Engine Manager Section */}
      <div className="mt-12 bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm">
        <div className="flex justify-between items-center pb-4 border-b border-slate-100 mb-6">
          <div>
            <h3 className="text-lg font-black text-slate-800">Minhas Reservas / Marcações</h3>
            <p className="text-xs text-slate-400 mt-0.5">Acompanhe os seus agendamentos, horários de atendimento e histórico geral.</p>
          </div>
          <span className="px-3 py-1 bg-rose-50 text-rose-700 rounded-full font-mono text-xs font-bold font-extrabold select-none">
        ...
        {bookingError && (
          <div className="mb-4 p-4 bg-rose-50 border border-rose-100 text-rose-800 rounded-2xl text-xs font-semibold">
            {bookingError}
          </div>
        )}
        ...
        {loadingBookings ? (
          ...
          <div className="py-12 text-center space-y-3">
            <div className="inline-flex items-center justify-center w-12 h-12 bg-rose-50 rounded-2xl text-rose-600">
            ...
            <a href="/explore" className="inline-block px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold shadow-sm cursor-pointer mt-2 text-center">
                  ...
                  return (
                    <tr key={bk.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-4">
                        ...
                        <div className="text-[10px] font-mono font-bold text-rose-600 mt-0.5">⏱ {bk.start_time} - {bk.end_time}</div>
                      </td>
                      ...
                      <td className="py-4 text-right flex items-center justify-end gap-1 px-1">
                        ...
                        {bk.booking_status !== 'cancelled' && bk.booking_status !== 'no_show' && bk.booking_status !== 'completed' && (
                          <button
                            ...
                            className="px-2 py-1 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg text-[10px] font-bold cursor-pointer transition-colors border border-rose-100"
                          >
                            Cancelar
                          </button>
                        )}
      ...
      <div className="mt-12 bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm text-left">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-5 border-b border-slate-100 mb-6 gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-rose-50 border border-rose-100 rounded-full text-xs font-semibold text-rose-600 mb-2">
        ...
        {redeemError && (
          <div className="mb-5 p-4 bg-rose-55 bg-rose-50 border border-rose-100 text-rose-855 text-rose-800 rounded-2xl text-xs font-bold font-mono">
            {redeemError}
          </div>
        )}
          ...
          <div className="bg-slate-50 border border-slate-100 p-6 rounded-3xl space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <button
                ...
                className="flex-1 bg-slate-900 border border-transparent hover:bg-rose-700 disabled:opacity-45 text-white py-3 px-4 rounded-xl text-xs font-bold tracking-wide transition-all cursor-pointer text-center"
              >
              ...
              <button
                ...
                className="flex-grow bg-rose-600 border border-transparent hover:bg-rose-700 disabled:opacity-45 text-white py-3 px-4 rounded-xl text-xs font-extrabold tracking-wide transition-all cursor-pointer text-center"
              >
                Resgatar 10.00€ <span className="block text-[9px] font-mono font-normal text-rose-200 tracking-wider mt-0.5">(Custo: 1000 Pontos)</span>
              </button>
            </div>
                ...
                <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-3.5 rounded-2xl border border-slate-150 gap-2 text-xs">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono bg-rose-50 border border-rose-200 px-2 py-0.5 rounded text-[10px] text-rose-700 font-black tracking-wide select-all">
                        {rw.code}
                      </span>
                      <button
                        onClick={() => {
      ...
      <div className="mt-8 bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm text-left">
        <div className="flex items-center gap-2 mb-2">
          <MessageSquare className="w-5 h-5 text-rose-600 animate-bounce" />
        ...
        <div className="fixed inset-0 z-50 bg-slate-905 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl border border-slate-100 relative animate-in fade-in zoom-in-95 duration-200 text-left">
            ...
            <p className="text-xs text-slate-505 text-slate-500 leading-relaxed mb-6">
              Classifique o serviço <span className="font-extrabold text-rose-600">{reviewBooking?.service?.name}</span> realizado em <span className="font-extrabold text-slate-800">{reviewBooking?.business?.name}</span>. A sua avaliação será imediatamente visível no salão!
            </p>
            ...
            <form onSubmit={handleSubmitReview} className="space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500 text-xs rounded-2xl leading-relaxed text-slate-800 placeholder-slate-400 resize-none font-sans"
                />
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2.5 pt-3">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-black uppercase tracking-wide transition shadow-md hover:shadow-lg hover:shadow-rose-100 cursor-pointer flex items-center justify-center gap-1.5 disabled:opacity-50"
                >
                  {submittingReview ? (
                    <>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl border border-slate-100 relative animate-in fade-in zoom-in-95 duration-200 text-left">
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-6">
              Abra um ticket de disputa para o atendimento <span className="font-extrabold text-rose-600">{disputeBooking?.service?.name}</span> em <span className="font-extrabold text-slate-800">{disputeBooking?.business?.name}</span>. A equipa Glamzo contactará as duas partes e analisará o estorno do Stripe.
            </p>
            ...
            <form onSubmit={handleSubmitDispute} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-rose-500 focus:outline-none text-xs rounded-xl text-slate-800 font-sans"
                >
