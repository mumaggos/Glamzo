...
export default function Account() {
  ...
  const handleSubmitDispute = (e: React.FormEvent) => {
    ...
    financeService.openDispute(disputeBooking.id, disputeBooking.business_id, disputeBooking.business?.name || 'Salão', user.id, clientName, 'customer', disputeReason, disputeDescription);
    setRedeemSuccess(`🚨 Reclamação registada. A equipa Glamzo abriu uma disputa. Analisaremos em 24h.`);
    setDisputeModalOpen(false); setSubmittingDispute(false);
  };
  ...
  return (
    <div id="account-view" className="min-h-screen bg-[#F8F9FC] font-sans pb-20">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 relative z-20">
        ...
        {/* Menu de Abas */}
        <div className="flex overflow-x-auto bg-white rounded-2xl shadow-lg border border-slate-100 p-2 gap-2 mb-8 no-scrollbar">
          {[
        ...
        {activeTab === 'reservas' && (
          <div className="space-y-6 animate-fade-in">
            <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/60">
              <h3 className="text-xl font-black text-slate-900 mb-6">Histórico de Agendamentos</h3>
              
              {bookingSuccess && <div className="mb-4 p-4 bg-emerald-50 text-emerald-700 rounded-xl text-sm font-bold flex items-center gap-2"><CheckCircle className="w-5 h-5" /> {bookingSuccess}</div>}
              {bookingError && <div className="mb-4 p-4 bg-rose-50 text-rose-700 rounded-xl text-sm font-bold flex items-center gap-2"><AlertCircle className="w-5 h-5" /> {bookingError}</div>}
              ...
              {loadingBookings ? (
                ...
                <div className="py-12 text-center bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <h4 className="text-base font-bold text-slate-800">Sem reservas</h4>
                  <p className="text-xs text-slate-500 mt-2 mb-4">Comece a cuidar de si marcando o seu primeiro serviço.</p>
                ...
                <div className="space-y-4">
                  {bookings.slice(0, visibleBookings).map(bk => {
                    ...
                    return (
                      <div key={bk.id} className="p-5 border border-slate-200 rounded-2xl flex flex-col md:flex-row gap-6 justify-between items-start md:items-center bg-white hover:border-purple-300 transition-colors">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-mono text-slate-400">#{bk.id.substring(0,8)}</span>
                          </div>
                          <h4 className="font-black text-slate-900 text-base">{bk.service?.name} <span className="text-slate-400 font-medium text-xs">em</span> {bk.business?.name}</h4>
                        ...
                        <div className="flex flex-col items-end gap-3 w-full md:w-auto border-t md:border-t-0 border-slate-100 pt-4 md:pt-0">
                          <span className="text-xl font-black text-purple-600 font-mono">{Number(bk.total_price).toFixed(2)}€</span>
